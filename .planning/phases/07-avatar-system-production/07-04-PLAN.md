...
must_haves:
  truths:
    ...
    - "Placeholder text ('I'm sending you a photo...') is appended to session history — NOT the raw tool_calls message (Pitfall 3)"
    ...
    - "User approves reference image → navigated to /chat (React Query cache invalidated)"
  artifacts:
    ...
    - path: "frontend/src/api/avatars.ts"
      provides: "createAvatar(), updateAvatar(), generateReferenceImage()"
      exports: ["createAvatar", "updateAvatar", "generateReferenceImage"]
...
**Changes to make (surgical — do not rewrite the entire file):**

1. Add SEND_PHOTO_TOOL definition near the top of the file (after the existing constants):
...
2. In `handle_message()`, the current LLM call for intimate mode is:
```python
reply = await self._llm.complete(history + [user_message], system_prompt)
```

Replace this LLM call block (the `try:` around it) with the new intimate-mode-aware version. The new version:
- In SECRETARY mode: same as before (no tools)
- In INTIMATE mode: pass `tools=[SEND_PHOTO_TOOL], tool_choice="auto"` to the LLM call

The existing `self._llm.complete()` signature does not support tools. The tool call is only needed for intimate mode, so call the OpenAI client directly in intimate mode with tools, while still using `self._llm.complete()` for secretary mode.

Replace the LLM call section (the `try:` block that calls `self._llm.complete()`) with:
...
No other changes to chat.py. Preserve all existing gates (crisis, content guard, mode switching, skill dispatch).
  ...
  <verify>
    ...
    <manual>Run existing test suite: `cd backend && python -m pytest tests/ -x -q 2>&1 | tail -5` — all existing tests must still pass (tool call addition is additive, not breaking).</manual>
...
<task type="auto">
  ...
  <action>
**`frontend/src/api/avatars.ts`** — Avatar API functions:

```typescript
/**
 * Avatar API client module.
 * createAvatar: POST /avatars — called on first setup during onboarding
 * updateAvatar: PATCH /avatars/me — called from SettingsPage for edits post-signup
...
export async function updateAvatar(token: string, data: Partial<AvatarData>): Promise<AvatarResponse> {
  const res = await fetch('/avatars/me', {
    method: 'PATCH',
    headers: {
      Authorization: `Bearer ${token}`,
...
**`frontend/src/pages/AvatarSetupPage.tsx`** — Onboarding form + reference image preview loop:

```tsx
/**
 * AvatarSetupPage — shown once during onboarding (before /chat access).
 * Also accessible from /avatar-setup for re-editing post-signup.
 *
 * Flow per CONTEXT.md locked decision:
 *   1. User fills in avatar details (all four AVTR-01 through AVTR-04 fields)
 *   2. Submit → create/update avatar → generate reference image
...
import { createAvatar, updateAvatar, generateReferenceImage, type AvatarData } from '../api/avatars'
...
export default function AvatarSetupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      // Create avatar (or update if already exists — server handles idempotency)
      try {
        await createAvatar(token, form)
      } catch (err: unknown) {
        // If avatar already exists (400), try update instead
        const msg = err instanceof Error ? err.message : ''
        if (msg.includes('already exists')) {
          await updateAvatar(token, form)
        } else {
          throw err
        }
      }
...
**`frontend/src/App.tsx`** — Add onboarding gate and new routes.
...
4. `ProtectedRoute` now includes onboarding gate: if no avatar → redirect to `/avatar-setup`
...
export default function SubscribePage() {
  return <div className="min-h-screen bg-gray-950 text-white flex items-center justify-center"><p>Subscribe — coming soon</p></div>
}
```

**`backend/app/routers/avatars.py`** — Add `POST /avatars/me/reference-image` endpoint. This endpoint calls the ImageProvider to generate a reference image immediately and returns the signed URL. Add after the existing `update_avatar` endpoint (added in Plan 02 Task 1):
...
```
  ...
  <verify>
    ...
    <manual>Verify: `python -c "from app.services.chat import SEND_PHOTO_TOOL, PHOTO_PLACEHOLDER_MSG; print('OK')"` passes. Frontend build shows zero TypeScript errors.</manual>
    <sampling_rate>run after task 2</sampling_rate>
  </verify>
  <done>
    SEND_PHOTO_TOOL in chat.py. Intimate mode LLM call detects tool_calls response and calls enqueue_photo_job(). Placeholder text appended to history (not raw tool_calls). AvatarSetupPage.tsx form with all 4 AVTR fields + reference image preview + regenerate loop. avatars.ts module with createAvatar/updateAvatar/generateReferenceImage. App.tsx OnboardingGate redirects to /avatar-setup when avatar is null. SubscribePage stub prevents TypeScript errors. POST /avatars/me/reference-image endpoint added to avatars.py router. Frontend build passes.
  </done>
</task>
...
<verification>
- `cd backend && python -c "from app.services.chat import SEND_PHOTO_TOOL; print(SEND_PHOTO_TOOL['function']['name'])"` → "send_photo"
- `cd backend && python -m pytest tests/ -x -q 2>&1 | tail -3` — all existing tests pass
- `cd frontend && npm run build 2>&1 | tail -5` — zero TypeScript errors
- `grep "avatar-setup" frontend/src/App.tsx` — route present
- `grep "OnboardingGate" frontend/src/App.tsx` — gate applied to /chat and /settings
- `grep "enqueue_photo_job" backend/app/services/chat.py` — wired in intimate mode
- `grep "reference-image" backend/app/routers/avatars.py` — endpoint present
</verification>
