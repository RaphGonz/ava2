...
**IMPORTANT — Read chat.py before writing anything.**
...
```python
# Already in __init__ (line ~101-102) — DO NOT redeclare:
self._openai_client = AsyncOpenAI(api_key=_settings.openai_api_key, max_retries=1)
self._intent_model = _settings.llm_model
```

`self._openai_client` is a raw `AsyncOpenAI` instance — it already exposes `.chat.completions.create()`.
`self._intent_model` is a string — accessible anywhere on `self` inside `handle_message()`.
...
1. Add `import json` at the top of the file (after the existing imports) — needed to parse tool call arguments. Skip this if `import json` is already present.
