...
must_haves:
  truths:
    ...
    - "PATCH /avatars/me updates gender, nationality, age, physical_description (avatar editable post-signup)"
  ...
  artifacts:
    ...
    - path: "backend/app/routers/avatars.py"
      provides: "PATCH /avatars/me full update endpoint"
      exports: ["router"]
...
Output: Updated avatar model + PATCH endpoint; complete Stripe billing router + subscription helpers; config fields added; `require_active_subscription` dependency wiring chat endpoints.
</objective>
...
<task type="auto">
  <name>Task 1: Avatar model extension + PATCH /avatars/me endpoint</name>
  <files>
    backend/app/models/avatar.py
    backend/app/routers/avatars.py
  </files>
  <action>
**`backend/app/models/avatar.py`** — Add `gender` and `nationality` fields to AvatarCreate and AvatarResponse. Add `AvatarUpdate` model for the PATCH endpoint. Keep existing fields unchanged.
...
class AvatarUpdate(BaseModel):
    """Used by PATCH /avatars/me — all fields optional, only provided fields updated."""
    name: Optional[str] = Field(None, min_length=1, max_length=100)
    age: Optional[int] = Field(None, ge=20)
...
**`backend/app/routers/avatars.py`** — Add three changes:
1. Import `AvatarUpdate` from models
2. Update `create_avatar` DB insert to include `gender` and `nationality`
3. Add new `PATCH /avatars/me` endpoint (full avatar edit — not just persona) for post-signup editing (CONTEXT.md: "Avatar editable at any time post-signup via web settings")
...
Add new endpoint after existing `update_persona`:
```python
...
async def update_avatar(
    ...
    """
    Update the authenticated user's avatar (any field).
    Used by SettingsPage for post-signup avatar editing.
    Editing triggers a new reference image on the frontend side (not this endpoint's concern).
    Clears session avatar cache so next message picks up new values immediately.
    """
    ...
    if "personality" in update_data:
        update_data["personality"] = update_data["personality"].value  # serialize enum

    result = db.from_("avatars").update(update_data).eq("user_id", str(user.id)).execute()
    if not result.data:
        raise HTTPException(status_code=404, detail="No avatar found")

    # Invalidate session avatar cache
...
```
  ...
  <verify>
    <automated>cd "C:/Users/raphg/Desktop/IA/ava2/backend" && python -c "from app.models.avatar import AvatarCreate, AvatarUpdate, AvatarResponse; a = AvatarCreate(name='Ava', age=25, personality='caring', gender='woman', nationality='French'); print('AvatarCreate OK:', a.gender, a.nationality); u = AvatarUpdate(gender='woman'); print('AvatarUpdate OK:', u.gender); r = AvatarResponse(id='x', user_id='y', name='Ava', age=25, personality='caring', created_at='2026-01-01T00:00:00'); print('AvatarResponse gender field:', r.gender)"</automated>
    <sampling_rate>run after task 1, before task 2</sampling_rate>
  </verify>
  <done>
    AvatarCreate accepts gender and nationality without error. AvatarUpdate model exists with all optional fields. AvatarResponse includes gender and nationality (nullable). PATCH /avatars/me endpoint registered in avatars router. create_avatar insert includes gender and nationality.
  </done>
</task>
...
<verification>
- `python -c "from app.models.avatar import AvatarCreate, AvatarUpdate; AvatarCreate(name='x', age=25, personality='caring', gender='woman', nationality='French')"` — no error
...
<success_criteria>
1. AvatarCreate, AvatarUpdate, AvatarResponse all include gender and nationality fields
2. POST /avatars insert includes gender and nationality columns
3. PATCH /avatars/me full update endpoint exists (for post-signup editing)
