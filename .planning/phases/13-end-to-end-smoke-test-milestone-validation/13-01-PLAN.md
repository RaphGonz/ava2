...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    | EMAI-03 | Receipt email in inbox within 60s of payment | PASS/FAIL | [email subject/timestamp] | HH:MM |
    ...
    | ADMN-01 | /admin/metrics shows 5 metrics with real data | PASS/FAIL | [metrics values] | HH:MM |
    ...
    4. Run `cd backend && python -m pytest tests/test_smoke_usage_events.py -x -v` to verify ADMN-02
    ...
    ---
    *Runbook version: 1.0 — created Phase 13 Plan 01*
    *Production URL: https://avasecret.org*
  </action>
  <verify>
  ...
  <done>
    test_smoke_usage_events.py runs without import errors (SKIP is acceptable if production DB not configured in local env). docs/smoke-test-runbook.md exists with all 28 evidence table rows covering every v1.1 requirement.
  </done>
</task>
...
<verification>
- `backend/tests/conftest.py` exports `unsubscribed_jwt` and `regular_user_jwt` fixtures that skip gracefully when env vars absent
...
- Existing test suite still passes: `cd backend && python -m pytest tests/ -x -q --ignore=tests/test_smoke_paywall.py --ignore=tests/test_smoke_usage_events.py`
</verification>

<success_criteria>
- All 3 smoke test files created and importable without errors
