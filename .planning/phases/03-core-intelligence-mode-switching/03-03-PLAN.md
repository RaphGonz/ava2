...
must_haves:
  ...
  artifacts:
    ...
    - path: "backend/app/services/chat.py"
      provides: "ChatService.handle_message() — orchestrates mode detection, session, LLM call, and response text"
      exports: ["ChatService"]
...
<objective>
Wire together the LLM service, session store, and mode detector into a ChatService orchestrator, then upgrade webhook.py to use it — replacing the Phase 2 echo with real AI responses.
...
<task type="auto">
  ...
  <action>
**`backend/app/services/user_lookup.py`** — add `get_avatar_for_user()` function after the existing `lookup_user_by_phone()`:
...
**`backend/app/services/chat.py`** — NEW file. ChatService orchestrates all Phase 3 logic:
...
Note: `object.__setattr__` is used to set `_avatar_cache` because `SessionState` is a frozen dataclass. This is the correct approach — do not add `avatar_cache` to `store.py` as that file is owned by Plan 03-02.
  </action>
  <verify>
...
**New webhook.py** — keep all existing GET verification handler and outer try/except structure intact, only replace `process_whatsapp_message`:
